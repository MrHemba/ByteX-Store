...
import { Laptop, Monitor, Printer, Cpu, ChevronRight, Shield, Award, MessageCircle, Clock } from "lucide-react";
  ...
  return (
    <>
      ...
      <main style={{ paddingTop: 80 }}>
        ...
        {/* ── HERO ── */}
        <section className="circuit-bg" style={{ padding: "80px 24px 72px", textAlign: "center", position: "relative", overflow: "hidden" }}>
          {/* Glow orbs */}
          <div style={{ position: "absolute", top: "10%", left: "10%", width: 400, height: 400, borderRadius: "50%",
            background: "radial-gradient(circle, rgba(0,102,255,0.12) 0%, transparent 70%)", pointerEvents: "none" }} />
          <div style={{ position: "absolute", top: "20%", right: "5%", width: 300, height: 300, borderRadius: "50%",
            background: "radial-gradient(circle, rgba(0,200,255,0.1) 0%, transparent 70%)", pointerEvents: "none" }} />


<div style={{ position: "relative", zIndex: 1, maxWidth: 800, margin: "0 auto" }}>
<h1 style={{ fontSize: "clamp(32px,6vw,68px)", fontWeight: 900, lineHeight: 1.05,
              marginBottom: 20, letterSpacing: "-0.02em" }}>
              POTENCIA TU<br />
              <span style={{ background: "linear-gradient(135deg, var(--accent) 0%, var(--accent-2) 100%)",
            ...
            <p style={{ fontSize: 17, color: "var(--text-2)", marginBottom: 36, lineHeight: 1.6 }}>
              Descubre la tecnología del futuro hoy.
            </p>
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
              ...
              <a href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER}?text=${encodeURIComponent("Hola! Quiero información sobre los equipos disponibles")}`}
        ...
        {/* ── TRUST STRIP ── */}
        <div style={{ background: "var(--bg-elevated)", borderTop: "1px solid var(--border-solid)", borderBottom: "1px solid var(--border-solid)" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto", padding: "14px 24px",
            display: "flex", justifyContent: "center", gap: 40, flexWrap: "wrap" }}>
            {[
              { icon:<Shield size={15}/>, label:"Revisados" },
              { icon:<Award size={15}/>,  label:"Garantizados" },
              { icon:<MessageCircle size={15}/>, label:"Soporte WhatsApp" },
              { icon:<Clock size={15}/>, label:"Envíos a nivel nacional" },
            ].map(t=>(
              <div key={t.label} style={{ display:"flex", alignItems:"center", gap:7 }}>
                <span style={{ color:"var(--accent)" }}>{t.icon}</span>
                <span style={{ fontSize:12, fontWeight:600, color:"var(--text-2)" }}>{t.label}</span>
              </div>
            ))}
          </div>
        </div>
